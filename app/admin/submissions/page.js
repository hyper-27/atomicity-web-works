...
export default function AdminSubmissionsPage() {
  ...
  useEffect(() => {
    if (!authLoading && user) {
      // Only fetch if auth check is done and user is logged in
      setLoading(true);
      ...
      const unsubscribeSnapshot = onSnapshot(
        ...
        (err) => {
          ...
          setError(
            "Failed to load contact submissions. Please check permissions.",
          );
          setLoading(false);
        },
      );
            ...
            {Array(3)
              ...
              .map(
                (
                  ...
                  <div
                    key={index}
                    className="bg-gray-50 p-6 rounded-lg shadow-md border-l-4 border-indigo-600 flex flex-col sm:flex-row justify-between items-start sm:items-center"
                  >
                ...
                <div className="mb-4 sm:mb-0 sm:pr-4 flex-grow">
                  <p className="text-sm text-gray-500 mb-1">
                    {submission.createdAt?.toDate
                      ? format(submission.createdAt.toDate(), "PPP, p")
                      : "N/A"}
                  </p>
